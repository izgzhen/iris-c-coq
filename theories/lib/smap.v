...
Instance smap_lookup `{EqDecision K} A: Lookup K A (smap K A) :=
  λ k,
  fix go m :=
  match m with
...
Instance smap_partial_alter `{EqDecision K} A: PartialAlter K A (smap K A) :=
  λ f k,
  fix go m :=
    match m with
      | (k', v')::m' =>
...
Instance smap_omap `{EqDecision K} : OMap (smap K) :=
  λ A B f,
  fix go m :=
    match m with
      | (k, v)::m' =>
...
Instance smap_finmap `{EqDecision K} : FinMap K (smap K).
