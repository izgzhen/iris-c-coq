...
Lemma tac_wp_assign_offset Δ Δ' Δ'' E i b (o off: nat) (v1 v2 v2': val) (t1 t2 t2': type) Φ Φret:
  ...
  (Δ'' ⊢ Φ Vvoid) →
  Δ ⊢ WP curs (Sassign (Evalue (Vptr (b, (o + off)%nat))) (Evalue v2')) @ E {{ Φ ; Φret }}.
Proof.
  intros. eapply wand_apply.
  { iIntros "HP HQ". iApply wp_assign_offset; [done|done|done|].
    iSplitL "HP"; eauto. }
  rewrite into_laterN_env_sound -later_sep envs_simple_replace_sound //; simpl.
  rewrite right_id. apply later_mono, sep_mono_r, wand_mono=>//.
Qed.
...
Tactic Notation "wp_assign" :=
  ...
  | |- _ ⊢ wp ?E (curs (Sassign (Evalue (Vptr (?b, (?o + ?off)%nat))) _)) ?P ?Q =>
    iMatchHyp (fun H P => match P with ((b, o) ↦{_} _ @ ?t)%I =>
      eapply tac_wp_assign_offset;
