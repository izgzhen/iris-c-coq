...
\infer[wp-ret]
{}{\wpre{k'(v); KS}[\mask]{\pred} \proves \wpre{k(\erete{v}); \kcall{k'}::KS}[\mask]{\pred}}
...
\infer[wp-call]
{}
{ f \tmapsto \Funct(\tau, ps, e) * \later (\wpre{e[ps/ls]; \kcall{k}::KS}[\mask]{\pred})
...
\infer[wp-while]
{}{ \later \wpre{\eif{c}{e;\econtinue}{\ebreak}; \kwhile{c}{e}{k}::KS}[\mask]{\pred}
    \proves \wpre{ k(\ewhile{c}{e}; KS }[\mask]{\pred} }

\infer[wp-break]
{}{ \wpre{k'(\void); KS}[\mask]{\pred}
    \proves \wpre{k(\ebreak); \kwhile{c}{e}{k'}::KS}[\mask]{\pred} }

\infer[wp-continue]
{}{ \wpre{k'(\ewhile{c}{e}); KS}[\mask]{\pred}
    \proves \wpre{k(\econtinue); \kwhile{c}{e}{k'}::KS}[\mask]{\pred} }
